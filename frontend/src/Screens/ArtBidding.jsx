...
const ArtBidding = () => {
  ...
  const filteredArts = arts.filter((art) => {
    const nameMatch = art.name && art.name.toLowerCase().includes(searchQuery.toLowerCase());
    const artistMatch = art.artist && art.artist.toLowerCase().includes(searchQuery.toLowerCase());
    const priceMatch = art.start_price && String(art.start_price).includes(searchQuery);
    return nameMatch || artistMatch || priceMatch;
  });
    ...
    <div className="bidding-container">
      ...
      {selectedArt && (
        <div className="bid-section">
          <h2>Bidding on {selectedArt.name}</h2>
          <input
            type="number"
            value={bidAmount}
